...
d1 %>% filter(!is.na(por_score_std),
              !is.na(abs_score_std),
              !is.na(spirit_score_std)) %>% 
  distinct(subject_id) %>%
  left_join(d1_byquestion %>%
              select(subject_id, s1_var_por, s1_var_abs, 
                     s1_var_spirit, s1_var_other) %>%
              distinct()) %>%
  gather(question, response, -subject_id) %>%
  mutate(scale = case_when(
    question %in% s1_var_por ~ "Porosity Vignettes",
    question %in% s1_var_abs ~ "Absorption",
    question %in% s1_var_spirit ~ "Spiritual Events",
...
```{r, include = F}
df_all <- data.frame()
for(i in s1_var_spirit){
  df_temp <- score_fun(df = d1_byquestion %>% 
                         select(country, subject_id, s1_var_spirit), 
                       var_omit = i) %>%
    mutate(omitted = i)
...
reg_check_fun <- function(df, var_omit){
  ...
  r0 <- lmer(score_std ~ por_score_std * abs_score_std +
...
reg_check_df %>%
  ...
  mutate(mod_type = case_when(omitted == "ALL VARIABLES INCLUDED" ~ "all",
         ...
         omitted = factor(omitted, 
                          levels = c("ALL VARIABLES INCLUDED", s1_var_spirit)),
         param = recode_factor(param,
                               "por_score_std" = "Porosity Vignettes",
                               "abs_score_std" = "Absorption",
                               "por_score_std:abs_score_std" = "Interaction")) %>%
...
reg_brms_check_fun <- function(df, var_omit){
  ...
  r0 <- brm(score_std ~ por_score_std * abs_score_std + 
              (1 | country/site/religion), data = d,
...
#   df = d1_byquestion %>% select(country, subject_id, s1_var_spirit),
#   var_omit = NA_character_)
# 
...
s1_subj_int_nodata <- d1_byquestion %>% 
  select(subject_id, s1_var_spirit, s1_var_other) %>%
  gather(question, response, -subject_id) %>%
...
d1r_irr <- full_join(
  d1_byquestion %>% 
    ...
    select(country, site, religion, subject_id, s1_var_spirit, s1_var_other) %>%
  ...
  mutate(agree = case_when(is.na(recoded) & is.na(response) ~ NA_real_,
                           ...
                           recoded != response ~ 0,
                           TRUE ~ NA_real_),
         question = factor(question, levels = c(s1_var_spirit, s1_var_other)),
...
```{r}
# s1r_perag_df %>% arrange(percent_agree)
# s1r_perag_df %>% count(reliability) %>% mutate(prop = n/sum(n))
s1r_perag_df %>% 
...
for(i in c(s1_var_spirit, s1_var_other)){
...
```{r}
rr1 <- lmer(spirit_recoded_std ~ por_score_std * abs_score_std +
...
regtab_fun(rr1) %>%
  ...
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr1)), color = "black")
```

```{r}
regtab_ran_fun(rr1) %>%
  mutate(Group = factor(Group,
                        levels = c("Country", "Site, nested within Country",
                                   "Religion, nested within Site, nested within Country", 
                                   "Residual"))) %>%
  arrange(Group) %>%
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr1)), color = "black")
```

### Original (limited scale)

```{r}
rr1a <- lmer(spirit_mod_score_std ~ por_score_std * abs_score_std +
               (1 | country/site/religion),
             data = d1r_mod)
summary(rr1a)
```
...
regtab_fun(rr1a) %>%
  ...
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr1a)), color = "black")
```
...
regtab_ran_fun(rr1a) %>%
  ...
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr1a)), color = "black")
```

### Recoded (limited scale)

```{r}
rr1b <- lmer(spirit_mod_recoded_std ~ por_score_std * abs_score_std +
...
regtab_fun(rr1b) %>%
  ...
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr1b)), color = "black")
```

XX BOOKMARK 
```{r}
regtab_ran_fun(rr1b) %>%
  # correct labeling by hand to account for different model
  mutate(Group = factor(Group, levels = c("Country",
                                          "Religion, nested within Country",
                                          "Residual"))) %>%
  arrange(Group) %>%
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr1b)), color = "black")
```
...
r1_comp_df <- bind_rows(
  ...
  mutate(Parameter = recode_factor(Parameter,
                                   ...
                                   "por_score_std" = "Porosity Vignettes", 
                                   "abs_score_std" = "Absorption",
                                   "por_score_std:abs_score_std" = "Porosity Vignettes × Absorption"))
```
...
rr1w <- lmer(spirit_recoded_std ~ por_score_std * abs_score_std +
               (1 + por_score_std * abs_score_std | country/site/religion), 
             data = d1r) # singular
rr1x <- lmer(spirit_recoded_std ~ por_score_std * abs_score_std +
               (1 + por_score_std * abs_score_std || country/site/religion), 
             data = d1r) # singular, failed to converge
rr1y <- lmer(spirit_recoded_std ~ por_score_std * abs_score_std +
               (1 + por_score_std + abs_score_std | country/site/religion), 
             data = d1r) # singular
rr1z <- lmer(spirit_recoded_std ~ por_score_std * abs_score_std +
               (1 + por_score_std + abs_score_std || country/site/religion), 
             data = d1r) # singular
```
...
```{r}
# rr1_bayes <- brm(spirit_recoded_std ~ por_score_std * abs_score_std +
...
```{r}
regtab_fun(rr1_bayes) %>%
  kable(align = c(rep("r", ncol(.) - 1), "l")) %>%
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr1_bayes)), color = "black")
```

```{r}
regtab_ran_fun(rr1_bayes) %>%
  mutate(Group = factor(Group,
                        levels = c("Country", "Site, nested within Country",
                                   "Religion, nested within Site, nested within Country", 
                                   "Residual"))) %>%
  ...
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr1_bayes)), color = "black")
```

### Original (limited scale)

```{r}
# rr1a_bayes <- brm(spirit_mod_score_std ~ por_score_std * abs_score_std +
...
```{r}
regtab_fun(rr1a_bayes) %>%
  kable(align = c(rep("r", ncol(.) - 1), "l")) %>%
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr1a_bayes)), color = "black")
```

```{r}
regtab_ran_fun(rr1a_bayes) %>%
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr1a_bayes)), color = "black")
```

### Recoded (limited scale)

```{r}
# rr1b_bayes <- brm(spirit_mod_recoded_std ~ por_score_std * abs_score_std +
#                   (1 | country/site/religion), d1r_mod,
#                 control = list(adapt_delta = 0.999),
#                 seed = 1234, cores = 4, chains = 4)
...
```{r}
regtab_fun(rr1b_bayes) %>%
  kable(align = c(rep("r", ncol(.) - 1), "l")) %>%
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr1b_bayes)), color = "black")
```

```{r}
regtab_ran_fun(rr1b_bayes) %>%
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr1b_bayes)), color = "black")
```
...
```{r}
rr2 <- lm(spirit_recoded_std ~ por_score_std * abs_score_std * country, d1r)
summary(rr2)
```
...
regtab_fun(rr2) %>%
  ...
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr2)), color = "black")
```
...
regtab_fun(rr3) %>%
  ...
  kable_styling(font_size = 16) %>%
  row_spec(2:3, bold = T) %>%
  row_spec(1:nrow(regtab_fun(rr3)), color = "black")
```
...
regtab_ran_fun(rr3) %>%
  ...
  kable(align = "r") %>%
  kable_styling(font_size = 16) %>%
  row_spec(1:nrow(regtab_ran_fun(rr3)), color = "black")
```
