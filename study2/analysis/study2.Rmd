...
coef(r2_dse) %>%
  ...
  full_join(sjstats::std_beta(r2_dse, type = "std") %>% 
              select(term, std.estimate) %>%
              rename(`β'` = std.estimate)) %>%
  full_join(sjstats::std_beta(r2_dse, type = "std2") %>% 
              select(term, std.estimate) %>%
              rename(`β''` = std.estimate)) %>%
...
coef(r2_spev) %>%
  ...
  rename(β = ".") %>%
  rownames_to_column("term") %>%
  full_join(sjstats::std_beta(r2_spev, type = "std") %>% 
              select(term, std.estimate) %>%
              rename(`β'` = std.estimate)) %>%
  full_join(sjstats::std_beta(r2_spev, type = "std2") %>% 
              select(term, std.estimate) %>%
              rename(`β''` = std.estimate)) %>%
  mutate_at(vars(-term), funs(format(round(., 2), nsmall = 2))) %>%
  kable(digits = 2, align = c("l", rep("r", 3))) %>%
