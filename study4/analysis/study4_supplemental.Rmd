...
# by scale, question
d4_byquestion %>%
  filter(subj %in% s4_sub, packet == 1) %>% 
  select(subj, s4_var_abs, s4_var_dse, s4_var_spev) %>%
...
for(i in c(levels(d4$country), "ALL")){
...
for(i in c(levels(d4$country), "ALL")){
  ...
  if(i == "China"){
    v <- s4_var_spev[s4_var_spev != "spev_07"]
  } else {
    v <- s4_var_spev
  }
...
```{r}
r1_dse_us <- lm(dse_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "US"))
summary(r1_dse_us)
```

```{r}
r1_dse_gh <- lm(dse_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "Ghana"))
summary(r1_dse_gh)
```

```{r}
r1_dse_th <- lm(dse_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "Thailand"))
summary(r1_dse_th)
```

```{r}
r1_dse_ch <- lm(dse_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "China"))
summary(r1_dse_ch)
```

```{r}
r1_dse_vt <- lm(dse_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "Vanuatu"))
summary(r1_dse_vt)
```
...
```{r}
r1_spev_us <- lm(spev_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "US"))
summary(r1_spev_us)
```

```{r}
r1_spev_gh <- lm(spev_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "Ghana"))
summary(r1_spev_gh)
```

```{r}
r1_spev_th <- lm(spev_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "Thailand"))
summary(r1_spev_th)
```

```{r}
r1_spev_ch <- lm(spev_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "China"))
summary(r1_spev_ch)
```

```{r}
r1_spev_vt <- lm(spev_score_std4 ~ abs_score_std4, 
                 d4 %>% filter(subj %in% s4_sub, country == "Vanuatu"))
summary(r1_spev_vt)
```
